...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "\n"
        "# --- STEP 6: Skip desktop welcome wizard (GNOME only) ---\n"
        "echo \"==== STEP 6: skip OOBE ====\"\n"
        "if [ -d /etc/gdm3 ] || command -v gdm3 >/dev/null 2>&1; then\n"
        "    # GNOME: skip gnome-initial-setup\n"
        "    if id \"$ASB_USER\" >/dev/null 2>&1; then\n"
        ...
        "    echo \"OK: GNOME OOBE skip\"\n"
        "fi\n"
        "\n"
        "# --- STEP 7: Auto-login via the detected display manager ---\n"
        "echo \"==== STEP 7: autologin ====\"\n"
        "if [ -d /etc/gdm3 ] || command -v gdm3 >/dev/null 2>&1; then\n"
        "    # GNOME (GDM3)\n"
        ...
        "[debug]\n"
        "EOF\n"
        "    echo \"OK: GDM3 autologin for $ASB_USER\"\n"
        "elif [ -d /etc/sddm.conf.d ] || command -v sddm >/dev/null 2>&1; then\n"
        "    # KDE Plasma (SDDM) — detect the session .desktop name\n"
        "    # Plasma 5: plasma.desktop; Plasma 6: plasmax11/plasmawayland.desktop\n"
        "    SDDM_SESSION=\"\"\n"
        "    for d in /usr/share/xsessions /usr/share/wayland-sessions; do\n"
        "        [ -d \"$d\" ] || continue\n"
        "        for s in plasma plasmax11 plasmawayland plasmacompositor; do\n"
        "            [ -f \"$d/$s.desktop\" ] && SDDM_SESSION=\"$s\" && break\n"
        "        done\n"
        "        [ -n \"$SDDM_SESSION\" ] && break\n"
        "    done\n"
        "    # Fallback: first .desktop in xsessions or wayland-sessions\n"
        "    if [ -z \"$SDDM_SESSION\" ]; then\n"
        "        for d in /usr/share/xsessions /usr/share/wayland-sessions; do\n"
        "            [ -d \"$d\" ] || continue\n"
        "            SDDM_SESSION=$(ls \"$d\"/*.desktop 2>/dev/null | head -1 | xargs -I{} basename {} .desktop)\n"
        "            [ -n \"$SDDM_SESSION\" ] && break\n"
        "        done\n"
        "    fi\n"
        "    [ -z \"$SDDM_SESSION\" ] && SDDM_SESSION=\"plasma\"\n"
        "    mkdir -p /etc/sddm.conf.d\n"
        "    cat > /etc/sddm.conf.d/00-appsandbox-autologin.conf <<EOF\n"
        "[Autologin]\n"
        "User=$ASB_USER\n"
        "Session=$SDDM_SESSION\n"
        "EOF\n"
        "    echo \"OK: SDDM autologin for $ASB_USER ($SDDM_SESSION)\"\n"
        "elif command -v lightdm >/dev/null 2>&1 || [ -d /etc/lightdm ]; then\n"
        "    # XFCE/MATE (LightDM)\n"
        "    mkdir -p /etc/lightdm/lightdm.conf.d\n"
        "    cat > /etc/lightdm/lightdm.conf.d/00-appsandbox-autologin.conf <<EOF\n"
        "[Seat:*]\n"
        "autologin-user=$ASB_USER\n"
        "autologin-user-timeout=0\n"
        "user-session=xfce\n"
        "EOF\n"
        "    echo \"OK: LightDM autologin for $ASB_USER (xfce)\"\n"
        "else\n"
        "    echo \"WARN: no known display manager found (gdm3/sddm/lightdm) — skipping autologin\"\n"
        "fi\n"
        "# Add the user to autologin groups where applicable\n"
        "groupadd -f autologin 2>/dev/null; usermod -aG autologin \"$ASB_USER\" 2>/dev/null && echo \"OK: autologin group\" || echo \"WARN: autologin group setup\"\n"
        "\n"
        "# ============================================================\n"
        ...
        "fi\n"
        "# Override asb_drm display params if the host staged a config marker.\n"
        "# /etc/appsandbox-display-config format: \"WxHxR\" e.g. \"2560x1440x144\"\n"
        "if [ -f /etc/appsandbox-display-config ]; then\n"
        "    DISP_CFG=$(tr -d '\\r\\n[:space:]' < /etc/appsandbox-display-config)\n"
        "    if [ -n \"$DISP_CFG\" ]; then\n"
        "        DISP_W=$(echo \"$DISP_CFG\" | cut -dx -f1)\n"
        "        DISP_H=$(echo \"$DISP_CFG\" | cut -dx -f2)\n"
        "        DISP_R=$(echo \"$DISP_CFG\" | cut -dx -f3)\n"
        "        if echo \"$DISP_CFG\" | grep -qE '^[0-9]+x[0-9]+x[0-9]+$'; then\n"
        "            printf 'blacklist hyperv_drm\\nblacklist simpledrm\\noptions asb_drm width=%s height=%s refresh=%s\\n' \\\n"
        "                \"$DISP_W\" \"$DISP_H\" \"$DISP_R\" > /etc/modprobe.d/asb_drm.conf \\\n"
        "              && echo \"OK: asb_drm.conf overridden to ${DISP_W}x${DISP_H}@${DISP_R}Hz\" \\\n"
        "              || echo \"FAIL: asb_drm.conf override\"\n"
        "        fi\n"
        "    fi\n"
        "    rm -f /etc/appsandbox-display-config 2>/dev/null || true\n"
        "fi\n"
        ...
        "fi\n"
        "if [ -f \"$EXTRAS/org.gnome.Shell-no-gpu.conf\" ] && command -v gnome-shell >/dev/null 2>&1; then\n"
        ...
        "IdleAction=ignore\n"
        "EOF\n"
        "# dconf for no GNOME blank / idle / lock (GNOME only).\n"
                      ...
                      const wchar_t *manifest_path)
{
     ...
     * Try casper/minimal.squashfs first (26.04+ desktop), then fall back
     * to casper/filesystem.squashfs (22.04 and earlier, Kubuntu, and
     * some derivatives that kept the legacy naming).
    ...
    if (GetFileAttributesW(sqfs_path) == INVALID_FILE_ATTRIBUTES) {
        log_msg(L"minimal.squashfs not found, trying filesystem.squashfs...");
        swprintf(sqfs_path, MAX_PATH, L"%c:\\casper\\filesystem.squashfs", iso_drive);
        if (GetFileAttributesW(sqfs_path) == INVALID_FILE_ATTRIBUTES) {
            log_err(L"No squashfs found on ISO (tried casper/minimal.squashfs "
                    L"and casper/filesystem.squashfs)");
            goto cleanup;
        }
    }
